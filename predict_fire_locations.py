...
if __name__ == "__main__":
    ...
    X_predict, actual_df = preprocess_data(config.ACTUAL_DATA_PATH, mode='predict')

    print("Loading trained model...")
    if not os.path.exists(config.LGBM_MODEL_PATH):
        raise FileNotFoundError(f"Trained model not found: {config.LGBM_MODEL_PATH}. Please train the model first.")
    ...
    os.makedirs(os.path.dirname(config.PREDICTED_CSV_PATH), exist_ok=True)
    if 'LAT' not in predicted_fire_locations_df.columns or 'LON' not in predicted_fire_locations_df.columns:
        ...
        predicted_fire_locations_df[['LAT', 'LON', 'FIRE_PROBABILITY']].to_csv(config.PREDICTED_CSV_PATH, index=False)
        print(f"Predicted fire locations saved to {config.PREDICTED_CSV_PATH}")
